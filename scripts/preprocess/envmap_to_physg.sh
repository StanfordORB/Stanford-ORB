...
GPU_INFO="--partition viscam --cpus_per_task 4 --mem 12G"
BASH_OUT_PATH="/viscam/projects/imageint/yzzhang/imageint/scripts/exp/preprocess_envmap_to_physg.txt"
for SCENE in \
...
--job physg_${SCENE} --command "$COMMAND" $GPU_INFO exclude=viscam1,viscam7,svl[1-6] |& tee -a $BASH_OUT_PATH
done
