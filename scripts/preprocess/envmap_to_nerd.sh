...
GPU_INFO="--partition svl --gpu_type titanrtx --cpus_per_task 8 --mem 12G"
BASH_OUT_PATH="/viscam/projects/imageint/yzzhang/imageint/scripts/exp/preprocess_envmap_to_nerd.txt"
for SCENE in \
...
--proj_dir /viscam/projects/imageint/yzzhang/imageint --conda_env neuralpil \
--env_vars PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python \
--job nerd_${SCENE} --command "$COMMAND" $GPU_INFO exclude=viscam1,viscam7,svl[1-6] |& tee -a $BASH_OUT_PATH
done
