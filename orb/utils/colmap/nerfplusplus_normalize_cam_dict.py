...
def normalize_cam_dict(in_cam_dict_file, out_cam_dict_file, target_radius=1., in_geometry_file=None, out_geometry_file=None):
    ...
    cam_dict, geometry = _normalize_cam_dict(cam_dict, target_radius, geometry)

    if geometry is not None:
        o3d.io.write_triangle_mesh(out_geometry_file, geometry)

    with open(out_cam_dict_file, 'w') as fp:
        json.dump(cam_dict, fp, indent=2, sort_keys=True)


def _normalize_cam_dict(in_cam_dict, target_radius=1., in_geometry=None):
    ...
    if in_geometry is not None:
        ...
        geometry_norm = in_geometry.transform(tf)
        out_geometry = geometry_norm
    else:
        out_geometry = None
