...
def validate(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS, denoiser):
    ...
    with open(os.path.join(out_dir, 'metrics.txt'), 'w') as fout:
        ...
        if os.getenv('NVDIFFRECMC_GT_ENV_MAP') == '1':
        ...
        for it, target in enumerate(dataloader_validate):
            ...
            if os.getenv('NVDIFFRECMC_GT_ENV_MAP') == '1':
                ...
                if 'scene_name' in frame and os.environ['NVDIFFRECMC_LIGHT_SCENE'] != frame['scene_name']:
                    continue
                ...
                arr = os.path.join(dataset_validate.base_dir.replace(scene, os.environ['NVDIFFRECMC_LIGHT_SCENE']), 'env_map', frame['file_path'].split('/')[-1]) + '.exr'
                ...
                lgt = light.EnvironmentLight(base=latlong_img)
...
if __name__ == "__main__":
    ...
    if os.path.splitext(FLAGS.ref_mesh)[1] == '.obj':
        ...
        if os.path.isdir(os.path.join(FLAGS.ref_mesh, 'train_mask')):
            dataset_train = DatasetCapture(os.path.join(FLAGS.ref_mesh, 'transforms_train.json'), FLAGS,
