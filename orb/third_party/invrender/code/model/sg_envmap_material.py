...
class EnvmapMaterialNetwork(nn.Module):
    ...
    def forward(self, points):
        ...
        if os.getenv('INVRENDER_GT_ENV_MAP_DEBUG') == '1':
            print('setting specular to 1, roughness to .1')
            ret['sg_specular_reflectance'] = torch.ones_like(ret['sg_specular_reflectance'])
            ret['sg_roughness'] = .1 * torch.ones_like(ret['sg_roughness'])
