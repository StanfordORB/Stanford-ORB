...
def eval_datasets(
    ...
    with strategy.scope():
        for dp in tqdm(df):
            ...
            if os.getenv('NERD_GT_ENV_MAP') == '1':
                novel_sgs = next(novel_sgs_iter)
                print('loading from', novel_sgs)
                novel_sgs = np.load(novel_sgs)
                if np.isnan(novel_sgs).any() or np.isinf(novel_sgs).any():
                    print('NaN or Inf in novel_sgs')
                    novel_sgs = np.zeros_like(novel_sgs)  # FIXME THIS IS WRONG NEED TO SKIP THIS IMAGE
                nerd.sgs_store.sgs.assign(novel_sgs)
            # Render image.
...
def render_video(
    ...
    for n, imgs in parameters.items():
        ...
        for i in range(imgs.shape[0]):
            ...
            # )
            Image.fromarray((imgs[i] * 255).astype(np.uint8).squeeze()).save(
                os.path.join(video_img_dir, "{}_{:06d}.png".format(n, i)),
            )
