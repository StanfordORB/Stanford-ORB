...
def gen_data(poses, imgs, img_paths, n_vali, outroot):
    ...
    n_imgs = imgs.shape[0]
    if isinstance(n_vali, (list, np.ndarray)):
        ind_vali = np.asarray(n_vali)
    else:
    ...
    for vi, i in enumerate(ind_train):
        ...
        if False:
            ...
            import os
            import json
            with open(os.path.join(os.path.dirname(img_paths[i]), '../../blender_format_LDR/transforms_train.json')) as f:
                frame_meta_blender = json.load(f)['frames'][vi]
    ...
    for vi, i in enumerate(ind_vali):
        ...
        if False:
            ...
            # import ipdb;
            # ipdb.set_trace()
            with open(os.path.join(os.path.dirname(img_paths[i]), '../../blender_format_LDR/transforms_test.json')) as f:
                frame_meta_blender = json.load(f)['frames'][vi]
            if os.path.basename(frame_meta_blender['file_path'] + '.png') != os.path.basename(img_paths[i]):
                print(frame_meta_blender['file_path'] + '.png', img_paths[i])
                import ipdb; ipdb.set_trace()
