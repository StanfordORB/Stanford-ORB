import pyexr
import sys
sys.path.insert(0, '/viscam/projects/imageint/yzzhang/imageint/imageint/third_party/singleimage')
import glob
...
from orb.third_party.singleimage import models, utils
from orb.utils.preprocess import load_mask_png, load_rgb_png, load_rgb_exr, cv2_downsize
...
SINGLEIMAGE_ROOT = os.path.join(PROJ_ROOT, 'imageint/third_party/singleimage')


class Pipeline(BasePipeline):
    def test_geometry(self, scene: str, overwrite=False):
        test_out_dir = os.path.join(SINGLEIMAGE_ROOT, 'evals/test', scene)
        normal_output_pattern = os.path.join(test_out_dir, '*_normal_2.exr')
        depth_output_pattern = os.path.join(test_out_dir, '*_depth_2.exr')
        if overwrite or len(glob.glob(normal_output_pattern)) == 0:
        ...
        return [
            ...
            for f1, f2, f3, f4 in zip(
                ...
                sorted(glob.glob(os.path.join('/viscam/projects/imageint/capture_scene_data/data/', scene, 'final_output/geometry_outputs/z_maps/*.npy'))),
                sorted(glob.glob(normal_output_pattern)),
                sorted(glob.glob(os.path.join('/viscam/projects/imageint/capture_scene_data/data/', scene, 'final_output/geometry_outputs/normal_maps/*.npy'))),
            )
        ]

    def test_geometry_core(self, scene):
        input_paths = list(sorted(glob.glob(os.path.join(SCENE_DATA_DIR, scene, f'final_output/blender_format_HDR/test/*.exr'))))
        input_mask_paths = list(sorted(glob.glob(os.path.join(SCENE_DATA_DIR, scene, f'final_output/blender_format_LDR/test_mask/*.png'))))
        test_out_dir = os.path.join(SINGLEIMAGE_ROOT, 'evals/test', scene)
        execute(input_paths, input_mask_paths, ['--output', test_out_dir, '--cuda', '--imageSize', '256'])

    def test_new_light(self, scene: str, overwrite: bool):
        return []

    def test_new_view(self, scene: str, overwrite: bool):
        return []

    def test_material(self, scene: str, overwrite=False):
        test_out_dir = os.path.join(SINGLEIMAGE_ROOT, 'evals/test', scene)
        output_pattern = os.path.join(test_out_dir, '*_albedo_512_2.png')
    ...
    def test_shape(self, scene, overwrite):
        return {'output_mesh': None, 'target_mesh': None}


def execute(imList, segList, args):
    ...
    parser.add_argument('--modelRootInit', default=None,
                        help='the directory where the initialization trained model is save')
    parser.add_argument('--modelRootsRefine', nargs='+', default=[None, None],
                        help='the directory where the refine models are saved')
    parser.add_argument('--modelRootGlob', default=None,
                        help='the directory where the global illumination model is saved')
    parser.add_argument('--epochIdInit', type=int, default=14, help='the training epoch of the initial network')
    parser.add_argument('--epochIdsRefine', nargs='+', type=int, default=[7, 5],
                        help='the training epoch of the refine network')
    parser.add_argument('--epochIdGlob', type=int, default=17,
                        help='the traing epoch of the global illuminationn prediction network')
    ...
    if opt.modelRootInit is None:
        opt.modelRootInit = os.path.join(SINGLEIMAGE_ROOT, 'check_initEnvGlob_cascade0')
    ...
    if len(opt.modelRootsRefine) != opt.cascadeLevel or opt.modelRootsRefine[0] is None:
        ...
        for n in range(1, opt.cascadeLevel + 1):
            root = os.path.join(SINGLEIMAGE_ROOT, 'check_cascadeEnvGlob')
    ...
    if opt.modelRootGlob is None:
        opt.modelRootGlob = os.path.join(SINGLEIMAGE_ROOT, 'check_globalillumination')
    ...
    # initalize tensors
    segBatch = Variable(
        torch.FloatTensor(opt.batchSize, 1, opt.imageSize, opt.imageSize))  # FIXME changed from 3 to 1
    ...
    for n in range(0, opt.cascadeLevel):
        encoderRefs[n].load_state_dict(
            torch.load('{0}/encoderRefs{1}_{2}.pth'.format(opt.modelRootsRefine[n], n + 1, opt.epochIdsRefine[n])))
        encoderRefs[n] = encoderRefs[n].eval()
        albedoRefs[n].load_state_dict(
            torch.load('{0}/albedoRefs{1}_{2}.pth'.format(opt.modelRootsRefine[n], n + 1, opt.epochIdsRefine[n])))
        albedoRefs[n] = albedoRefs[n].eval()
        normalRefs[n].load_state_dict(
            torch.load('{0}/normalRefs{1}_{2}.pth'.format(opt.modelRootsRefine[n], n + 1, opt.epochIdsRefine[n])))
        normalRefs[n] = normalRefs[n].eval()
        roughRefs[n].load_state_dict(
            torch.load('{0}/roughRefs{1}_{2}.pth'.format(opt.modelRootsRefine[n], n + 1, opt.epochIdsRefine[n])))
        roughRefs[n] = roughRefs[n].eval()
        depthRefs[n].load_state_dict(
            torch.load('{0}/depthRefs{1}_{2}.pth'.format(opt.modelRootsRefine[n], n + 1, opt.epochIdsRefine[n])))
        depthRefs[n] = depthRefs[n].eval()
        envRefs[n].load_state_dict(
            torch.load('{0}/envRefs{1}_{2}.pth'.format(opt.modelRootsRefine[n], n + 1, opt.epochIdsRefine[n])))
    ...
    for n in range(0, len(imList)):
        ...
        # imBg = (imBg / 255.0) ** (2.2)
        imBg = load_rgb_exr(imgName, downsize_factor=2048 / opt.imageSize)
        ...
        # if seg.shape[2] > 1:
        #     seg = seg[:, :, 0]
        # seg = (seg > 0.999).astype(dtype=int)
        seg = load_mask_png(segName, downsize_factor=2048 / opt.imageSize).astype(int)
        ...
        for n in range(0, opt.cascadeLevel):
            ...
            globalIllu2 = globIllu1to2(torch.cat([(2 * globalIllu1 - 1), \
                                                  albedoPred, normalPred, roughPred, depthPred, segBatch],
                                                 dim=1)).detach()
        ...
        imgName = os.path.join(opt.output, imgName.split('/')[-1].replace('.exr', '_input.png'))
        segName = imgName.replace('input', 'mask')
        ...
        for n in range(0, len(albedoPreds)):
            ...
            img = (0.5 * (albedoPreds[n] + 1) * segBatch.expand_as(albedoPreds[n]))[0, :, :, :].data.cpu().numpy()
            img = np.clip(img, 0, 1)
            img = (255 * img.transpose([1, 2, 0])).astype(np.uint8)
            img = Image.fromarray(img)
            img = img.resize(size=(BENCHMARK_RESOLUTION, BENCHMARK_RESOLUTION), resample=Image.BILINEAR)
            img.save(imgName.replace('input', f'albedo_{BENCHMARK_RESOLUTION}_{n}'))
        ...
        for n in range(0, len(normalPreds)):
            ...
            pyexr.write(normalName[-1].replace('.png', '.exr'), cv2_downsize(normalPreds[n].permute(0, 2, 3, 1).squeeze().cpu().numpy(), downsize_factor=opt.imageSize / BENCHMARK_RESOLUTION))
        ...
        for n in range(0, len(depthPreds)):
            ...
            depthName.append(imgName.replace('input', 'depth_%d' % n).replace('png', 'exr'))
            pyexr.write(depthName[-1], cv2_downsize(depthPreds[n].permute(0, 2, 3, 1).squeeze().cpu().numpy(), downsize_factor=opt.imageSize / BENCHMARK_RESOLUTION))
